...
return ( <label className="block space-y-2"> <span className="text-sm font-medium text-[#3d3832]">{label}</span> <div className="flex items-center gap-3">
<input
type="range"
min={1}
max={5}
...
/> <span className="w-8 text-center font-semibold text-coral">{value}</span> </div> <div className="flex justify-between text-xs text-[#8a8480]"> <span>{low}</span> <span>{high}</span> </div> </label>
);
}
...
return ( <fieldset className="border-0 p-0 m-0 space-y-2"> <legend className="text-sm font-medium text-[#3d3832]">{label}</legend> <div className="flex gap-2 flex-wrap">
{options.map((opt) => (
<button
key={String(opt.val)}
type="button"
